...
function renderCards(){
 // Injection du délai d'animation (Wow effect)
 const html=games.map((g,i)=>`<button class="game-card" data-game="${g.id}" style="animation-delay:${i*0.1}s"><div class="game-icon">${g.icon}</div><h3>${g.name}</h3><p>${g.desc}</p><span class="play">→</span></button>`).join("");
